...
using VelerSoftware.SZC35.Highlighting;
...
namespace VelerSoftware.SZC35
{
	...
	public class TextEditor : Control, ITextEditorComponent, IServiceProvider, IWeakEventListener
	{
		...
		protected TextEditor(TextArea textArea)
		{
            ...
            this.textArea.TextView.Services.AddService(typeof(VelerSoftware.SZC35.Services.ITextMarkerService), new VelerSoftware.SZC35.Services.TextMarkerService(this));
		...
		protected virtual void OnTextChanged(EventArgs e)
		{
			if (TextChanged != null) {
                ...
                {
                    if (this.isLineSingle == false)
                    {
                        if (this.Document.Text.Contains(char.ConvertFromUtf32(10)))
                        {     
                            this.Document.Text = this.Document.Text.Replace(char.ConvertFromUtf32(10), null);
                        }
                        if (this.Document.Text.Contains(char.ConvertFromUtf32(13)))
                        {
                            this.Document.Text = this.Document.Text.Replace(char.ConvertFromUtf32(13), null);
                        }
                    }
        ...
        #region SpellCheck
        /// <summary>
        /// Spell Check dependency property.
        /// </summary>
        public static readonly DependencyProperty SpellCheckProperty =
            DependencyProperty.Register("SpellCheck", typeof(bool), typeof(TextEditor),
                                        new FrameworkPropertyMetadata(Boxes.True));

        /// <summary>
        /// Specifies whether the text editor uses Spell Checking.
        /// </summary>
        public bool SpellCheck
        {
            get { return (bool)GetValue(SpellCheckProperty); }
            set { 
                SetValue(SpellCheckProperty, Boxes.Box(value));
                if (Variables.SpellCheck) 
                    UpdateSpellCheck();
            }
        }

        public System.Windows.Controls.SpellingError GetSpellingError(int index)
        {
            if (Variables.SpellCheck && this.TextArea.Caret.Offset > 0)
            {
                return this.SpellCheckTextBox.GetSpellingError(index);
            }
            else { return null; }
        }

        public int GetSpellingErrorLength(int index)
        {
            if (Variables.SpellCheck && this.TextArea.Caret.Offset > 0)
            {
                return this.SpellCheckTextBox.GetSpellingErrorLength(index);
            }
            else { return -1; }
        }

        public int GetSpellingErrorStart(int index)
        {
            if (Variables.SpellCheck && this.TextArea.Caret.Offset > 0)
            {
                return this.SpellCheckTextBox.GetSpellingErrorStart(index);
            }
            else { return -1; }
        }

        private System.Windows.Controls.TextBox SpellCheckTextBox = new System.Windows.Controls.TextBox();
        ...
        /// <summary>
        /// isLineSingle dependency property.
        /// </summary>
        public static readonly DependencyProperty isLineSingleProperty =
            DependencyProperty.Register("isLineSingle", typeof(bool), typeof(TextEditor),
                                        new FrameworkPropertyMetadata(Boxes.True));
        public bool isLineSingle
        {
            get { return (bool)GetValue(isLineSingleProperty); }
            set { SetValue(isLineSingleProperty, Boxes.Box(value)); }
        }
