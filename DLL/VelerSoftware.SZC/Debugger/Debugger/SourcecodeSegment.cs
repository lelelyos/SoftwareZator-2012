...
namespace VelerSoftware.SZC.Debugger.Debugger
{
    public class SourcecodeSegment : DebuggerObject
    {
        ...
        /// </summary>
        private static IEnumerable<string> RelocateSymURL(Module module, string symUrl)
        {
        ...
        private static ISymUnmanagedDocument GetSymDocumentFromFilename(Module module, string filename, byte[] checksum)
        {
            ...
            if (Path.IsPathRooted(filename))
            {
                foreach (ISymUnmanagedDocument symDoc in module.SymDocuments)
                {
                    ...
                    {
                        if (string.Equals(url.Replace("\\Temp\\Building", "\\Temp\\Debugger"), filename, StringComparison.OrdinalIgnoreCase))
                            return symDoc;
                    }
                }
            }
        ...
        private static string GetFilenameFromSymDocument(Module module, ISymUnmanagedDocument symDoc)
        {
