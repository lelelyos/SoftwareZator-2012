...
using VelerSoftware.SZC.Debugger.Debugger.Mono.Cecil.Signatures;
...
namespace VelerSoftware.SZC.Debugger.Debugger.MetaData
{
    public class DebugMethodInfo : System.Reflection.MethodInfo, IDebugMemberInfo, IOverloadable
    {
        ...
        private MethodDefSig MethodDefSig
        {
            get
            {
        ...
        private void LoadBackingFieldToken()
        {
        ...
        private static bool Read(List<byte> code, params byte[] expected)
        {
        ...
        // Read field token
        private static bool ReadToken(List<byte> code, ref uint token)
        {
            if (code.Count < 4)
                return false;
        ...
        private static Value GetLocalVariableValue(StackFrame context, ISymUnmanagedVariable symVar)
        {
