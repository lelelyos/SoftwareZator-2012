...
namespace VelerSoftware.SZC.Debugger.Debugger
{
    ...
    public class Eval : DebuggerObject
    {
        ...
        public Value Result
        {
            get
            {
                switch (this.State)
                {
                    ...
                    case EvalState.EvaluatedTimeOut:
                        if (Variables.Langue == "en")
                        {
                            throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Timeout_EN);
                        }
                        else
                        {
                            throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Timeout);
                        }
                    default:
                        if (Variables.Langue == "en")
                        {
                            throw new DebuggerException(VelerSoftware.SZC.Properties.Resources.Eval_Unknow_State_EN);
                        }
                        else
                        {
                            throw new DebuggerException(VelerSoftware.SZC.Properties.Resources.Eval_Unknow_State);
                        }
                }
            }
        }
        ...
        private Eval(AppDomain appDomain, string description, EvalStarter evalStarter)
        {
            ...
            catch (COMException e)
            {
                if ((uint)e.ErrorCode == 0x80131C26)
                {
                    if (Variables.Langue == "en")
                    {
                        throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Optimized_EN);
                    }
                    else
                    {
                        throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Optimized);
                    }
                }
                else if ((uint)e.ErrorCode == 0x80131C28)
                {
                    if (Variables.Langue == "en")
                    {
                        throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Wrong_Appdomain_EN);
                    }
                    else
                    {
                        throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Wrong_Appdomain);
                    }
                }
                else if ((uint)e.ErrorCode == 0x8013130A)
                {
                    ...
                    if (Variables.Langue == "en")
                    {
                        throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_IL_EN);
                    }
                    else
                    {
                        throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_IL);
                    }
                }
                ...
                else if ((uint)e.ErrorCode == 0x80131C22)
                {
                    ...
                    if (Variables.Langue == "en")
                    {
                        throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Stack_Overflow_EN);
                    }
                    else
                    {
                        throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Stack_Overflow);
                    }
                }
        ...
        private static Thread GetEvaluationThread(AppDomain appDomain)
        {
            ...
            if (Variables.Langue == "en")
            {
                throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Suitable_Thread_EN);
            }
            else
            {
                throw new GetValueException(VelerSoftware.SZC.Properties.Resources.Eval_Suitable_Thread);
            }
        }
        ...
        */
        ...
        private static ICorDebugValue[] ValuesAsCorDebug(Value[] values)
        {
